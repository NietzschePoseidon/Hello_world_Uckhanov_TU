# 🗿 Лабораторная работа №1
*Добро пожаловать в мой первый учебный репозиторий! Здесь я учусь работать с системой контроля версий Git, настраиваю рабочую среду и пишу базовый код на Python.*
***
## ⚙ Что было сделано:
✔ *Зарегистрирован аккаунт на <span style="color: #D97575;">GitHub.*  
✔ *Настроен редактор <span style="color: #D97575;">VS Code</span> (установлены расширения для <span style="color: #D97575;">Python</span>).*  
✔ *Создан первый удаленный репозиторий.*  
✔ *Изучен синтаксис разметки <span style="color: #D97575;">Markdown.*  
***
## 💻 Пример кода на Python
*В ходе работы я написал скрипт, который выполняет простые математические действия:*
```python
name = input("")

a = int(input())
b = int(input())

result = a + b

print(f"Привет, {name}!")
print(f"Результат сложения {a} + {b} равен {result}")
```
